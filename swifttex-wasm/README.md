...
We provide out-of-the-box components for popular frameworks to make integration seamless.
...
function App() {
  return (
    <main>
      ...
      <h2>Block Math:</h2>
      <SwiftTeX tex="\int_0^\infty e^{-x^2} dx = \frac{\sqrt{\pi}}{2}" display_mode={true} />
    </main>
  );
}
```
...
<template>
  <main>
    <h2>Inline Math: <SwiftTeX tex="E = mc^2" /></h2>
    <h2>Block Math:</h2>
    <SwiftTeX tex="\int_0^\infty e^{-x^2} dx = \frac{\sqrt{\pi}}{2}" displayMode />
  </main>
</template>
...
<main>
  <h2>Inline Math: <SwiftTeX tex="E = mc^2" /></h2>
  <h2>Block Math:</h2>
  <SwiftTeX tex="\int_0^\infty e^{-x^2} dx = \frac{\sqrt{\pi}}{2}" displayMode={true} />
</main>
```

### Web Components (Vanilla HTML)

Register the `<swift-tex>` custom element to use it natively in any HTML document without a framework!
